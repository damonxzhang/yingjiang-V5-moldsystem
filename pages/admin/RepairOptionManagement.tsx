...
import { MOCK_REPAIR_OPTIONS } from '../../services/mockData';
...
const RepairOptionManagement: React.FC = () => {
  const [options, setOptions] = useState<RepairOption[]>(MOCK_REPAIR_OPTIONS);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingOption, setEditingOption] = useState<RepairOption | null>(null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">维修选项管理</h2>
          <p className="text-slate-500 text-sm">维护模具维修时可供选择的标准故障处理内容</p>
        </div>
        <button 
          onClick={handleAdd}
          className="px-5 py-2.5 bg-red-600 text-white font-bold rounded-xl text-sm shadow-lg shadow-red-100 flex items-center gap-2 hover:bg-red-700 transition-all"
        >
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-red-50/30">
              <h3 className="text-lg font-bold text-slate-800">
                {options.find(o => o.id === editingOption.id) ? '编辑维修选项' : '新增维修选项'}
              </h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-500 transition-all"
                  placeholder="如：更换顶针"
                />
              </div>
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-500 transition-all appearance-none"
                >
                  <option value="机械故障">机械故障</option>
                  <option value="电气故障">电气故障</option>
                  <option value="模面损伤">模面损伤</option>
                  <option value="合模异常">合模异常</option>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-500 transition-all resize-none"
            ...
            <div className="p-6 border-t border-slate-100 flex gap-3">
              ...
              <button 
                onClick={saveOption}
                className="flex-1 px-4 py-2.5 bg-red-600 text-white font-bold rounded-xl text-sm shadow-lg shadow-red-100 hover:bg-red-700 transition-all"
              >
                保存
              </button>
            </div>
...
export default RepairOptionManagement;
